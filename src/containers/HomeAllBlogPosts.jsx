...
const HomeAllBlogPosts = () => {
  ...
  const { data, error, isLoading } = useGetPostQuery(page);
  const posts = data;
  ...
  return (
    <section className="my-10 p-5 lg:p-0">
      ...
      <div className="flex flex-col tablet:flex-row tablet:justify-between my-5">
        <button onClick={prevPage} className="text-text self-center tablet:self-start flex justify-center items-center gap-1">
          <img src={arrowLeftBlack} alt="arrow left black icon" />
          <span>Previous</span>
        </button>
        <button onClick={nextPage} className="text-text self-center tablet:self-start flex justify-center items-center gap-1">
          <span>Next</span>
